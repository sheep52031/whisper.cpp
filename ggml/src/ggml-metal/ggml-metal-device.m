...
ggml_metal_library_t ggml_metal_library_init(ggml_metal_device_t dev) {
    ...
    // TODO: move to a function
    {
        ...
        // 🔧 FIX: Always use bundleForClass as SWIFTPM_MODULE_BUNDLE is not reliable here
        ...
        GGML_LOG_INFO("%s: [DEBUG] Initial path_lib: %s\n", __func__, path_lib ? [path_lib UTF8String] : "nil");

        // 🔧 FIX: Try to find the nested bundle generated by SwiftPM (WhisperPackage_Whisper.bundle)
        if (path_lib == nil) {
            NSString * bundleName = @"WhisperPackage_Whisper";
            NSURL * nestedBundleURL = [bundle URLForResource:bundleName withExtension:@"bundle"];
            
            GGML_LOG_INFO("%s: [DEBUG] Searching for nested bundle '%s'...\n", __func__, [bundleName UTF8String]);
            
            if (nestedBundleURL) {
                GGML_LOG_INFO("%s: [DEBUG] Found nested bundle URL: %s\n", __func__, [[nestedBundleURL path] UTF8String]);
                NSBundle * nestedBundle = [NSBundle bundleWithURL:nestedBundleURL];
                path_lib = [nestedBundle pathForResource:@"default" ofType:@"metallib"];
            }
        }
        
        // 🔧 FIX: Hardcoded fallback for XPC Service structure
        // Path: TaiwanWhisperService.xpc/Contents/MacOS/TaiwanWhisperService
        // Target: TaiwanWhisperService.xpc/Contents/Resources/WhisperPackage_Whisper.bundle/Contents/Resources/default.metallib
        if (path_lib == nil) {
            NSString * bin_cur = [[NSProcessInfo processInfo] arguments][0];
            NSString * bin_dir = [bin_cur stringByDeletingLastPathComponent]; // .../Contents/MacOS
            
            // Check ../Resources/WhisperPackage_Whisper.bundle/Contents/Resources/default.metallib
            NSString * xpc_resource_path = [bin_dir stringByAppendingPathComponent:@"../Resources/WhisperPackage_Whisper.bundle/Contents/Resources/default.metallib"];
            if ([[NSFileManager defaultManager] fileExistsAtPath:xpc_resource_path]) {
                path_lib = [xpc_resource_path stringByStandardizingPath];
                GGML_LOG_INFO("%s: [DEBUG] Found default.metallib via XPC relative path: %s\n", __func__, [path_lib UTF8String]);
            }
        }
        ...
        if (path_lib == nil) {
            ...
            if ([mainBundlePath containsString:@".xpc"]) {
                ...
                NSString * debugBuildDir = [[[[mainBundlePath stringByDeletingLastPathComponent] // XPCServices
                ...
                NSString * candidate = [debugBuildDir stringByAppendingPathComponent:@"default.metallib"];
        ...
        if (path_lib == nil) {
             ...
             NSString * candidate = [exeDir stringByAppendingPathComponent:@"default.metallib"];
             if ([[NSFileManager defaultManager] fileExistsAtPath:candidate]) {
                 path_lib = candidate;
             }
        }
        ...
        if (path_lib != nil) {
            ...
            if (path_resource) {
                ...
                // 🔧 FIX: Try to find the nested bundle generated by SwiftPM (WhisperPackage_Whisper.bundle) for source file
                if (path_source == nil) {
                    NSString * bundleName = @"WhisperPackage_Whisper";
                    NSURL * nestedBundleURL = [bundle URLForResource:bundleName withExtension:@"bundle"];
                    ...
                    if (nestedBundleURL) {
                        NSBundle * nestedBundle = [NSBundle bundleWithURL:nestedBundleURL];
                        path_source = [nestedBundle pathForResource:@"ggml-metal" ofType:@"metal"];
                    }
                }
                
                // Fallback for local development (Hardcoded source path)
                if (path_source == nil) {
                    NSString * localSourcePath = @"/Users/jason/Developer/taiwan_talka/LocalPackages/WhisperPackage/whisper_cpp_src/ggml/src/ggml-metal/ggml-metal.metal";
                    if ([[NSFileManager defaultManager] fileExistsAtPath:localSourcePath]) {
                ...
                if (path_source == nil) {
                    NSString * bin_cur = [[NSProcessInfo processInfo] arguments][0];
                    NSString * bin_dir = [bin_cur stringByDeletingLastPathComponent];
                    NSString * xpc_resource_path = [bin_dir stringByAppendingPathComponent:@"../Resources/WhisperPackage_Whisper.bundle/Contents/Resources/ggml-metal.metal"];
                    if ([[NSFileManager defaultManager] fileExistsAtPath:xpc_resource_path]) {
