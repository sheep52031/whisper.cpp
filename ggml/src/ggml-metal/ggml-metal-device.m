...
#if !TARGET_CPU_X86_64 && TARGET_OS_OSX &&                                     \
...
int ggml_metal_pipeline_max_theads_per_threadgroup(
...
ggml_metal_library_t ggml_metal_library_init(ggml_metal_device_t dev) {
  ...
  // TODO: move to a function
  {
    ...
    // 🔧 FIX: Always use bundleForClass as SWIFTPM_MODULE_BUNDLE is not
    // reliable here
    ...
    GGML_LOG_INFO("%s: [DEBUG] Bundle path: %s\n", __func__,
                  [[bundle bundlePath] UTF8String]);
    GGML_LOG_INFO("%s: [DEBUG] Initial path_lib: %s\n", __func__,
                  path_lib ? [path_lib UTF8String] : "nil");

    // 🔧 FIX: Try to find the nested bundle generated by SwiftPM
    // (WhisperPackage_Whisper.bundle)
    if (path_lib == nil) {
      // Try multiple bundle names for different projects
      NSArray<NSString *> *bundleNames = @[
        @"WhisperPackage_Whisper",
        @"VoiceBearWhisperPackage_VoiceBearWhisper" // VoiceBear-iOS
      ];

      for (NSString *bundleName in bundleNames) {
        GGML_LOG_INFO("%s: [DEBUG] Searching for nested bundle '%s'...\n",
                      __func__, [bundleName UTF8String]);

        NSURL *nestedBundleURL = [bundle URLForResource:bundleName
                                          withExtension:@"bundle"];
        if (nestedBundleURL) {
          GGML_LOG_INFO("%s: [DEBUG] Found nested bundle URL: %s\n", __func__,
                        [[nestedBundleURL path] UTF8String]);
          NSBundle *nestedBundle = [NSBundle bundleWithURL:nestedBundleURL];
          path_lib = [nestedBundle pathForResource:@"default"
                                            ofType:@"metallib"];
          if (path_lib)
            break;
        }
    ...
    // 🔧 FIX: Hardcoded fallback for XPC Service structure
    // Path: TaiwanWhisperService.xpc/Contents/MacOS/TaiwanWhisperService
    // Target:
    // TaiwanWhisperService.xpc/Contents/Resources/WhisperPackage_Whisper.bundle/Contents/Resources/default.metallib
    if (path_lib == nil) {
      NSString *bin_cur = [[NSProcessInfo processInfo] arguments][0];
      NSString *bin_dir =
          [bin_cur stringByDeletingLastPathComponent]; // .../Contents/MacOS

      // Check
      // ../Resources/WhisperPackage_Whisper.bundle/Contents/Resources/default.metallib
      NSString *xpc_resource_path =
          [bin_dir stringByAppendingPathComponent:
                       @"../Resources/WhisperPackage_Whisper.bundle/Contents/"
                       @"Resources/default.metallib"];
      if ([[NSFileManager defaultManager] fileExistsAtPath:xpc_resource_path]) {
        path_lib = [xpc_resource_path stringByStandardizingPath];
        GGML_LOG_INFO(
            "%s: [DEBUG] Found default.metallib via XPC relative path: %s\n",
            __func__, [path_lib UTF8String]);
      }
    }

    if (path_lib == nil) {
      // Try to find the resource in the directory where the current binary
      // located.
      NSString *bin_cur = [[NSProcessInfo processInfo] arguments][0];
      NSString *bin_dir = [bin_cur stringByDeletingLastPathComponent];

      NSString *path_lib_default =
          [NSString pathWithComponents:@[ bin_dir, @"default.metallib" ]];
      if ([[NSFileManager defaultManager]
              isReadableFileAtPath:path_lib_default]) {
        GGML_LOG_INFO("%s: found '%s'\n", __func__,
                      [path_lib_default UTF8String]);

        NSDictionary *atts = [[NSFileManager defaultManager]
            attributesOfItemAtPath:path_lib_default
                             error:&error];
        if (atts && atts[NSFileType] == NSFileTypeSymbolicLink) {
          // Optionally, if this is a symlink, try to resolve it.
          path_lib_default = [[NSFileManager defaultManager]
              destinationOfSymbolicLinkAtPath:path_lib_default
                                        error:&error];
          if (path_lib_default && [path_lib_default length] > 0 &&
              ![[path_lib_default substringToIndex:1] isEqualToString:@"/"]) {
            // It is a relative path, adding the binary directory as directory
            // prefix.
            path_lib_default =
                [NSString pathWithComponents:@[ bin_dir, path_lib_default ]];
          }
          if (!path_lib_default || ![[NSFileManager defaultManager]
                                       isReadableFileAtPath:path_lib_default]) {
    ...
    // Fallback for XPC Service in Debug mode (Search in Build/Products/Debug)
    if (path_lib == nil) {
      NSString *mainBundlePath = [[NSBundle mainBundle] bundlePath];
      if ([mainBundlePath containsString:@".xpc"]) {
        // .../Debug/TaiwanWhisperApp.app/Contents/XPCServices/TaiwanWhisperService.xpc
        NSString *debugBuildDir =
            [[[[mainBundlePath stringByDeletingLastPathComponent] // XPCServices
                stringByDeletingLastPathComponent]                // Contents
                stringByDeletingLastPathComponent]                // App.app
                stringByDeletingLastPathComponent];               // Debug

        NSString *candidate =
            [debugBuildDir stringByAppendingPathComponent:@"default.metallib"];
        if ([[NSFileManager defaultManager] fileExistsAtPath:candidate]) {
          path_lib = candidate;
          GGML_LOG_INFO("%s: found default.metallib in Debug dir: %s\n",
                        __func__, [path_lib UTF8String]);
        }
      }
    }
    ...
    if (path_lib == nil) {
      // Try to find it in the directory of the executable
      // This can happen when running the tests
    ...
    if (path_lib != nil) {
      ...
      if (path_resource) {
        ...
        // 🔧 FIX: Try to find the nested bundle generated by SwiftPM for source
        // file
        if (path_source == nil) {
          // Try multiple bundle names for different projects
          NSArray<NSString *> *bundleNames = @[
          ...
          for (NSString *bundleName in bundleNames) {
            ...
            if (nestedBundleURL) {
              NSBundle *nestedBundle = [NSBundle bundleWithURL:nestedBundleURL];
              path_source = [nestedBundle pathForResource:@"ggml-metal"
                                                   ofType:@"metal"];
              if (path_source) {
                GGML_LOG_INFO(
                    "%s: [DEBUG] Found ggml-metal.metal in bundle: %s\n",
                    __func__, [bundleName UTF8String]);
                break;
              }
            }
        ...
        // Fallback for local development (Hardcoded source path)
        if (path_source == nil) {
          NSString *localSourcePath =
              @"/Users/jason/Developer/taiwan_talka/LocalPackages/"
              @"WhisperPackage/whisper_cpp_src/ggml/src/ggml-metal/"
              @"ggml-metal.metal";
          if ([[NSFileManager defaultManager]
                  fileExistsAtPath:localSourcePath]) {
            path_source = localSourcePath;
            GGML_LOG_INFO("%s: found ggml-metal.metal in local source: %s\n",
                          __func__, [path_source UTF8String]);
          }
        }

        // 🔧 FIX: Hardcoded fallback for XPC Service structure (Source file)
        if (path_source == nil) {
          NSString *bin_cur = [[NSProcessInfo processInfo] arguments][0];
          NSString *bin_dir = [bin_cur stringByDeletingLastPathComponent];
          NSString *xpc_resource_path =
              [bin_dir stringByAppendingPathComponent:
                           @"../Resources/WhisperPackage_Whisper.bundle/"
                           @"Contents/Resources/ggml-metal.metal"];
          if ([[NSFileManager defaultManager]
                  fileExistsAtPath:xpc_resource_path]) {
            path_source = [xpc_resource_path stringByStandardizingPath];
          }
        }
      }

      if (path_source == nil) {
        GGML_LOG_WARN("%s: error: could not use bundle path to find "
      ...
      src = [NSString stringWithContentsOfFile:path_source
                                      encoding:NSUTF8StringEncoding
                                         error:&error];
      if (error) {
        GGML_LOG_ERROR("%s: error: %s\n", __func__,
    ...
    if (!library) {
      @autoreleasepool {
        ...
        library = [device newLibraryWithSource:src
                                       options:options
                                         error:&error];
        if (error) {
          GGML_LOG_ERROR("%s: error: %s\n", __func__,
...
ggml_metal_library_init_from_source(ggml_metal_device_t dev, const char *source,
                                    bool verbose) {
  if (source == NULL) {
...
ggml_metal_pipeline_t ggml_metal_library_get_pipeline(ggml_metal_library_t lib,
                                                      const char *name) {
  return ggml_metal_pipelines_get(lib->pipelines, name);
}

ggml_metal_pipeline_t
ggml_metal_library_compile_pipeline(ggml_metal_library_t lib, const char *base,
                                    const char *name, ggml_metal_cv_t cv) {
  // note: the pipelines are cached in the library per device, so they are
  // shared across all metal contexts
  ...
  @autoreleasepool {
    ...
    res->obj = [lib->device newComputePipelineStateWithFunction:mtl_function
                                                          error:&error];
    ...
    if (res->obj.maxTotalThreadsPerThreadgroup == 0 ||
        res->obj.threadExecutionWidth == 0) {
      ggml_critical_section_end();
...
ggml_metal_encoder_t ggml_metal_encoder_init(ggml_metal_cmd_buf_t cmd_buf_raw,
                                             bool concurrent) {
...
void ggml_metal_encoder_debug_group_push(ggml_metal_encoder_t encoder,
                                         const char *name) {
  [encoder->obj
      pushDebugGroup:[NSString stringWithCString:name
                                        encoding:NSUTF8StringEncoding]];
}
...
void ggml_metal_encoder_set_bytes(ggml_metal_encoder_t encoder, void *data,
                                  size_t size, int idx) {
  [encoder->obj setBytes:data length:size atIndex:idx];
}

void ggml_metal_encoder_set_buffer(ggml_metal_encoder_t encoder,
                                   struct ggml_metal_buffer_id buffer,
                                   int idx) {
  [encoder->obj setBuffer:buffer.metal offset:buffer.offs atIndex:idx];
}

void ggml_metal_encoder_set_threadgroup_memory_size(
    ggml_metal_encoder_t encoder, size_t size, int idx) {
  [encoder->obj setThreadgroupMemoryLength:size atIndex:idx];
}

void ggml_metal_encoder_dispatch_threadgroups(ggml_metal_encoder_t encoder,
                                              int tg0, int tg1, int tg2,
                                              int tptg0, int tptg1, int tptg2) {
  [encoder->obj dispatchThreadgroups:MTLSizeMake(tg0, tg1, tg2)
...
struct ggml_metal_device {
  ...
  // technically not needed for devices with unified memory, but enables
  // discrete GPUs support ref: https://github.com/ggml-org/llama.cpp/pull/15906
...
ggml_metal_device_t ggml_metal_device_init(void) {
  ...
  if (dev->mtl_device == nil) {
    ...
    if (dev->mtl_device) {
      ...
      dev->props.has_simdgroup_reduction =
          [dev->mtl_device supportsFamily:MTLGPUFamilyApple7];
      dev->props.has_simdgroup_reduction |=
          [dev->mtl_device supportsFamily:MTLGPUFamilyMetal3_GGML];

      dev->props.has_simdgroup_mm =
          [dev->mtl_device supportsFamily:MTLGPUFamilyApple7];
      dev->props.has_unified_memory = dev->mtl_device.hasUnifiedMemory;

      dev->props.has_bfloat =
          [dev->mtl_device supportsFamily:MTLGPUFamilyMetal3_GGML];
      dev->props.has_bfloat |=
          [dev->mtl_device supportsFamily:MTLGPUFamilyApple6];
      if (getenv("GGML_METAL_BF16_DISABLE") != NULL) {
        dev->props.has_bfloat = false;
      }

      dev->props.has_tensor =
          [dev->mtl_device supportsFamily:MTLGPUFamilyMetal4_GGML];
      if (getenv("GGML_METAL_TENSOR_DISABLE") != NULL) {
        dev->props.has_tensor = false;
      }

      // note: disable the tensor API by default for old chips because with the
      // current implementation it is not useful
      // - M2 Ultra:   ~5% slower
      // - M4, M4 Max: no significant difference
      //
      // TODO: try to update the tensor API kernels to at least match the
      // simdgroup performance
      if (getenv("GGML_METAL_TENSOR_ENABLE") == NULL &&
        ...
        GGML_LOG_WARN(
            "%s: tensor API disabled for pre-M5 and pre-A19 devices\n",
            __func__);
        dev->props.has_tensor = false;
      }
      ...
      if (dev->props.has_tensor) {
        const char *src_tensor_f16 =
            ...
            " \n"
            "    auto cT = mm.get_destination_cooperative_tensor<decltype(tA), "
            "decltype(tB), float>(); \n"
            " \n"
            ...
            "    mm.run(sB, sA, cT); \n"
            " \n"
            "    auto tC = tensor<device float, dextents<int32_t, 2>, "
            "tensor_inline>(C, dextents<int32_t, 2>(4, 4)); \n"
            " \n"
            "    cT.store(tC); \n"
            "}";
        ...
        ggml_metal_library_t lib =
            ggml_metal_library_init_from_source(dev, src_tensor_f16, false);
        if (lib == NULL) {
          GGML_LOG_WARN("%s: - the tensor API is not supported in this "
                        "environment - disabling\n",
                        __func__);
          dev->props.has_tensor = false;
        } else {
          ggml_metal_pipeline_t ppl = ggml_metal_library_compile_pipeline(
              lib, "dummy_kernel", "dummy_kernel", nil);
          if (!ppl) {
            GGML_LOG_WARN("%s: - the tensor API is not supported in this "
                          "environment - disabling\n",
                          __func__);
      ...
      // try to compile a dummy kernel to determine if the tensor API is
      // supported for bfloat
      if (dev->props.has_tensor && dev->props.has_bfloat) {
        ...
        ggml_metal_library_t lib =
            ggml_metal_library_init_from_source(dev, src_tensor_bf16, false);
        if (lib == NULL) {
          GGML_LOG_WARN("%s: - the tensor API does not support bfloat - "
                        "disabling bfloat support\n",
                        __func__);
          dev->props.has_bfloat = false;
        } else {
          ggml_metal_pipeline_t ppl = ggml_metal_library_compile_pipeline(
      ...
      dev->props.supports_gpu_family_apple7 =
          [dev->mtl_device supportsFamily:MTLGPUFamilyApple7];

      dev->props.max_buffer_size = dev->mtl_device.maxBufferLength;
      dev->props.max_working_set_size =
          dev->mtl_device.recommendedMaxWorkingSetSize;
      dev->props.max_theadgroup_memory_size =
          dev->mtl_device.maxThreadgroupMemoryLength;

      strncpy(dev->props.name, [[dev->mtl_device name] UTF8String],
              sizeof(dev->props.name) - 1);
      ...
      // https://developer.apple.com/metal/Metal-Feature-Set-Tables.pdf
      {
        ...
        for (int i = MTLGPUFamilyMetal3_GGML + 5; i >= MTLGPUFamilyMetal3_GGML;
             --i) {
          if ([dev->mtl_device supportsFamily:i]) {
      ...
      GGML_LOG_INFO("%s: simdgroup reduction   = %s\n", __func__,
                    dev->props.has_simdgroup_reduction ? "true" : "false");
      GGML_LOG_INFO("%s: simdgroup matrix mul. = %s\n", __func__,
                    dev->props.has_simdgroup_mm ? "true" : "false");
      GGML_LOG_INFO("%s: has unified memory    = %s\n", __func__,
                    dev->props.has_unified_memory ? "true" : "false");
      GGML_LOG_INFO("%s: has bfloat            = %s\n", __func__,
                    dev->props.has_bfloat ? "true" : "false");
      GGML_LOG_INFO("%s: has tensor            = %s\n", __func__,
                    dev->props.has_tensor ? "true" : "false");
      GGML_LOG_INFO("%s: use residency sets    = %s\n", __func__,
                    dev->props.use_residency_sets ? "true" : "false");
      GGML_LOG_INFO("%s: use shared buffers    = %s\n", __func__,
                    dev->props.use_shared_buffers ? "true" : "false");
...
void ggml_metal_device_get_memory(ggml_metal_device_t dev, size_t *free,
                                  size_t *total) {
  if (@available(macOS 10.12, iOS 16.0, *)) {
...
bool ggml_metal_device_supports_op(ggml_metal_device_t dev,
  ...
  case GGML_OP_L2_NORM:
    return has_simdgroup_reduction &&
           (op->ne[0] % 4 == 0 && ggml_is_contiguous_1(op->src[0]));
  case GGML_OP_ARGMAX:
    return has_simdgroup_reduction;
  case GGML_OP_NORM:
  ...
  case GGML_OP_FLASH_ATTN_EXT:
    ...
    if (op->src[0]->ne[0] != 32 && op->src[0]->ne[0] != 40 &&
        op->src[0]->ne[0] != 64 && op->src[0]->ne[0] != 72 &&
        op->src[0]->ne[0] != 80 && op->src[0]->ne[0] != 96 &&
        op->src[0]->ne[0] != 112 && op->src[0]->ne[0] != 128 &&
        op->src[0]->ne[0] != 192 && op->src[0]->ne[0] != 256) {
      return false;
    }
...
const struct ggml_metal_device_props *
ggml_metal_device_get_props(ggml_metal_device_t dev) {
  return &dev->props;
}
...
struct ggml_metal_buffer {
  ...
  // if false, the Metal buffer data is allocated in private GPU memory and is
  ...
  // note: cannot use explicity "id<MTLResidencySet>" here because it is not
  // available on certain OSes
  id rset;
...
static void ggml_metal_log_allocated_size(id<MTLDevice> device,
                                          size_t size_aligned) {
  ...
  if (@available(macOS 10.12, iOS 16.0, *)) {
    ...
    if (device.currentAllocatedSize > device.recommendedMaxWorkingSetSize) {
      GGML_LOG_WARN("%s: warning: current allocated size is greater than the "
...
ggml_metal_buffer_t ggml_metal_buffer_init(ggml_metal_device_t dev, size_t size,
                                           bool shared) {
  ...
  // allocate shared buffer if the device supports it and it is required by the
  // buffer type
  if (shared) {
    ...
    res->all_data = (void *)atomic_fetch_add_explicit(
        &g_addr_device, size_aligned, memory_order_relaxed);
    res->is_shared = false;
  }
  ...
  if (res->all_data != NULL) {
    ...
    if (size_aligned > 0) {
      if (props_dev->use_shared_buffers && shared) {
        res->buffers[0].metal =
            [res->device newBufferWithBytesNoCopy:res->all_data
                                           length:size_aligned
                                          options:MTLResourceStorageModeShared
                                      deallocator:nil];
      } else {
        res->buffers[0].metal =
            [res->device newBufferWithLength:size_aligned
  ...
  if (size_aligned > 0 &&
      (res->all_data == NULL || res->buffers[0].metal == nil)) {
...
ggml_metal_buffer_t ggml_metal_buffer_map(ggml_metal_device_t dev, void *ptr,
  ...
  if (size_aligned <= props_dev->max_buffer_size) {
    ...
    if (size_aligned > 0) {
      res->buffers[res->n_buffers].metal =
          [res->device newBufferWithBytesNoCopy:ptr
                                         length:size_aligned
                                        options:MTLResourceStorageModeShared
                                    deallocator:nil];
    ...
    const size_t size_ovlp =
        ((max_tensor_size + size_page - 1) / size_page + 1) *
    ...
    for (size_t i = 0; i < size; i += size_step) {
      ...
      if (size_step_aligned > 0) {
        res->buffers[res->n_buffers].metal =
            [res->device newBufferWithBytesNoCopy:(void *)((uint8_t *)ptr + i)
...
#if TARGET_OS_OSX
    vm_deallocate((vm_map_t)mach_task_self(), (vm_address_t)buf->all_data,
                  buf->all_size);
#else
    free(buf->all_data);
#endif
...
void *ggml_metal_buffer_get_base(ggml_metal_buffer_t buf) {
  if (!buf) {
    GGML_LOG_ERROR("%s: error: buffer is NULL\n", __func__);
    return NULL;
  }
  return buf->all_data;
}
...
bool ggml_metal_buffer_is_shared(ggml_metal_buffer_t buf) {
  return buf ? buf->is_shared : false;
}

void ggml_metal_buffer_memset_tensor(ggml_metal_buffer_t buf,
...
void ggml_metal_buffer_set_tensor(ggml_metal_buffer_t buf,
  ...
  @autoreleasepool {
    ...
    id<MTLBuffer> buf_src =
        [buf->device newBufferWithBytesNoCopy:data_ptr
                                       length:size
                                      options:MTLResourceStorageModeShared
                                  deallocator:nil];
    ...
    // note: for experimentation purposes, here we use a semaphore to wait for
    // the copy to complete
    //       this is alternative to waitUntilCompleted, which should be faster,
    //       but don't seem to make much difference
...
void ggml_metal_buffer_get_tensor(ggml_metal_buffer_t buf,
  ...
  @autoreleasepool {
    ...
    id<MTLBuffer> buf_dst =
        [buf->device newBufferWithBytesNoCopy:data
                                       length:size
                                      options:MTLResourceStorageModeShared
                                  deallocator:nil];
...
ggml_metal_buffer_get_id(ggml_metal_buffer_t buf, const struct ggml_tensor *t) {
  ...
  for (int i = 0; i < buf->n_buffers; ++i) {
    ...
    // GGML_LOG_INFO("ioffs = %10ld, tsize = %10ld, sum = %10ld,
    // buf->buffers[%d].size = %10ld\n", ioffs, tsize, ioffs + tsize, i,
    // buf->buffers[i].size);
